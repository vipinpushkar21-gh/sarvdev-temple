// Script to add 12 Jyotirlinga temples to database
// Run with: node scripts/add-jyotirlingas.js
...
async function addJyotirlingas() {
  try {
    ...
    for (let i = 0; i < jyotirlingas.length; i++) {
      const temple = jyotirlingas[i];
      console.log(`${i + 1}. Adding ${temple.title}...`);
      
      const response = await fetch('http://localhost:3001/api/temples', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        console.log(`   ✅ ${temple.title} added successfully`);
      } else {
        console.log(`   ❌ Failed to add ${temple.title}`);
      }
    ...
    console.log('\n🎉 All 12 Jyotirlingas added successfully!');
    console.log('\nYou can now view them at:');
    console.log('- All temples: http://localhost:3001/temples');
    console.log('- Sacred categories: http://localhost:3001/sacred-categories');
