...
const completeLyrics = `नवग्रह चालीसा (Navgrah Chalisa)
॥ दोहा ॥
...
async function updateNavgrahChalisa() {
  try {
    await mongoose.connect(MONGODB_URI);
    console.log('Connected to MongoDB\n');

    const result = await Devotional.updateOne(
      { title: 'Navgrah Chalisa', deity: 'Navgrah' },
      { 
        $set: { 
          lyrics: completeLyrics,
          description: 'Navgrah Chalisa - Complete Hindi text with doha, chaupai, and summary.',
          updatedAt: new Date()
        } 
      }
    );
    ...
    if (result.matchedCount > 0) {
      console.log(`✓ Updated Navgrah Chalisa with complete lyrics`);
      console.log(`  Modified: ${result.modifiedCount} document(s)`);
      
      // Verify the update
      const updated = await Devotional.findOne({ title: 'Navgrah Chalisa' });
      console.log(`\n✓ Verification:`);
      console.log(`  Lyrics length: ${updated.lyrics.length} characters`);
      console.log(`  Contains '॥ इति श्री नवग्रह चालीसा ॥': ${updated.lyrics.includes('॥ इति श्री नवग्रह चालीसा ॥')}`);
    } else {
    ...
    console.error('Error updating Navgrah Chalisa:', error);
    process.exit(1);
  }
}

updateNavgrahChalisa();
