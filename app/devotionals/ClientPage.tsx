...
export default function ClientPage() {
  ...
  const languages = useMemo(() => {
    ...
    devotionals.forEach((d) => {
      if (d.language) setVals.add(d.language)
    })
  ...
  const deities = useMemo(() => {
    const setVals = new Set<string>()
    devotionals.forEach((d) => {
      if (d.deity) setVals.add(d.deity)
    })
    return ['all', ...Array.from(setVals).sort()]
  }, [devotionals])
  ...
  const categoriesWithCounts = useMemo(() => {
    ...
    // Start with allowed base categories only
    const base = FULL_CATEGORIES.filter((c) => !EXCLUDED_CATEGORY_IDS.has(c.id))
    // Add any categories present in data, except excluded ones
    for (const id of Array.from(seen)) {
      if (EXCLUDED_CATEGORY_IDS.has(id)) continue
        ...
        <div className="relative z-10">
              ...
              <div className="flex flex-col md:flex-row md:flex-wrap md:items-center md:justify-between gap-4">
                ...
                <div className="w-full md:w-auto flex gap-2 items-center">
                  ...
                  <label className="flex items-center gap-2 ml-2 text-sm text-text">
                    <input type="checkbox" className="accent-primary-500 h-4 w-4" checked={showTransliteration} onChange={(e) => setShowTransliteration(e.target.checked)} />
                    Show English transliteration
                  </label>
                  <label className="flex items-center gap-2 ml-2 text-sm text-text">
                    <input type="checkbox" className="accent-primary-500 h-4 w-4" checked={groupByLanguage} onChange={(e) => setGroupByLanguage(e.target.checked)} />
                    Group by language
                  </label>
                </div>
              </div>
          ...
          <section>
            ...
            {filteredDevotionals.length === 0 ? (
              <div className="text-center py-12 text-text text-lg font-bold">No devotionals found.</div>
            ) : (
              <>
                {groupByLanguage ? (
                  <div>
                    {languageOrder.map((lang) => {
                      ...
                      return (
                        <div key={lang} className="mb-10">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {group.map((d) => (
                              <Link key={d._id} href={`/devotionals/${createSlug(d.title || '')}`} className="focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-400">
                                <DevotionalCard
                                  title={d.title || ''}
                                  titleNode={(function () {
...
function FeaturedGrid({ devotionals }: { devotionals: Devotional[] }) {
  ...
  if (featured.length === 0) {
    return <div className="text-center py-6 text-text">No featured devotionals yet.</div>
  }
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {featured.map((d) => (
        <Link key={d._id} href={`/devotionals/${createSlug(d.title || '')}`} className="focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-400">
          <DevotionalCard title={d.title || ''} category={d.category} language={d.language} type={d.type} description={d.description} />
        </Link>
      ))}
    </div>
  )
...
function highlightText(text: string, term: string) {
  ...
  return parts.map((p, i) => (p.toLowerCase() === t.toLowerCase() ? (
