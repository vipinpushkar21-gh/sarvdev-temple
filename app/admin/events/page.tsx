...
export default function AdminEventsPage() {
  ...
  if (loading) return <div className="space-y-5"><h1 className="admin-page-title">Events</h1><div className="animate-pulse space-y-3">{[...Array(5)].map((_, i) => <div key={i} className="h-12 bg-gray-100 rounded-xl" />)}</div></div>
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <Link href="/admin" className="admin-btn admin-btn-primary px-4 py-2 text-sm">+ New Event</Link>
      </div>
        ...
        <div className="overflow-x-auto">
          <table>
            ...
            <tbody>
              ...
              {paginated.length === 0 && <tr><td colSpan={6} className="px-5 py-10 text-center text-gray-400">No events found.</td></tr>}
            </tbody>
          </table>
        </div>
        {totalPages > 1 && (
