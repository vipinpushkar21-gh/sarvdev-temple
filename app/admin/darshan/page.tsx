...
export default function AdminDarshanPage() {
  ...
  if (loading) return <div className="space-y-4"><h1 className="text-2xl font-bold text-gray-900 dark:text-white">Darshan</h1><div className="animate-pulse space-y-3">{[...Array(5)].map((_, i) => <div key={i} className="h-12 bg-gray-200 dark:bg-gray-800 rounded-lg" />)}</div></div>

  return (
    <div className="space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Daily Darshan</h1>
          <p className="text-sm text-gray-500 mt-0.5">{rows.length} total · {filtered.length} shown</p>
        </div>
        <Link href="/admin" className="px-4 py-2 text-sm font-medium text-white bg-purple-600 hover:bg-purple-700 rounded-lg transition-colors">+ New Darshan</Link>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
                ...
                <tr key={r._id} className={`border-t border-gray-200 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700/50 ${selected.has(r._id) ? 'bg-orange-50/50 dark:bg-orange-950/30' : ''}`}>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex gap-1.5">
                      {r.media && <a href={r.media} target="_blank" rel="noopener noreferrer" className="px-2.5 py-1 text-xs font-medium rounded-lg bg-blue-100 text-blue-700 hover:bg-blue-200 dark:bg-blue-900 dark:text-blue-300">View</a>}
              ...
              {paginated.length === 0 && <tr><td colSpan={7} className="px-4 py-8 text-center text-gray-400">No darshan submissions found.</td></tr>}
            </tbody>
          </table>
        </div>
        {totalPages > 1 && (
