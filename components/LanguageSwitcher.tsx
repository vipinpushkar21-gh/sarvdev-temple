...
type Props = {
  /** 'floating' (default) keeps the existing fixed corner button; 'inline' renders a compact header-row trigger. */
  variant?: 'floating' | 'inline'
}

export default function LanguageSwitcher({ variant = 'floating' }: Props) {
  ...
  const dropdown = (
    <div className={`absolute right-0 w-48 rounded-xl border overflow-hidden shadow-elevated ${variant === 'inline' ? 'top-full mt-2' : 'bottom-full mb-2'}`}
      style={{ backgroundColor: 'var(--color-surface)', borderColor: 'var(--color-border)' }}
    >
      <div className="px-3 py-2 border-b" style={{ borderColor: 'var(--color-border)', backgroundColor: 'var(--color-sunken)' }}>
    ...
    return (
      <div ref={ref} className="relative" suppressHydrationWarning>
        ...
        >
          ...
          <span className="text-caption font-bold" suppressHydrationWarning>{current.label}</span>
        </button>
      </div>
    )
  }
  ...
  return (
    <div ref={ref} className="fixed bottom-20 right-4 z-50 xl:hidden" suppressHydrationWarning>
      {open && dropdown}
