  ...
  return (
    <section className="relative w-full h-[80vh] min-h-[500px] max-h-[800px] overflow-hidden bg-secondary-900">
      ...
      <button
        ...
        className="absolute left-5 sm:left-8 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full border border-white/30 text-white hover:bg-white/10 transition-all duration-200 flex items-center justify-center disabled:opacity-30"
        aria-label="Previous"
      >
      ...
      <button
        ...
        disabled={isTransitioning}
        className="absolute right-5 sm:right-8 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full border border-white/30 text-white hover:bg-white/10 transition-all duration-200 flex items-center justify-center disabled:opacity-30"
        aria-label="Next"
      >
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}>
      ...
      <div className="absolute bottom-0 left-0 right-0 flex gap-0.5">
        {temples.map((_, index) => (
          <button
            ...
            disabled={isTransitioning}
            className="flex-1 h-0.5 disabled:cursor-not-allowed transition-all duration-300"
            style={{ background: index === currentIndex ? '#FF9933' : 'rgba(255,255,255,0.25)' }}
      ...
      <div className="absolute top-6 right-6">
        <span className="text-body-sm font-semibold tabular-nums text-white/90">
          {String(currentIndex + 1).padStart(2, '0')}
        </span>
        <span className="text-body-sm text-white/40 mx-1.5">/</span>
