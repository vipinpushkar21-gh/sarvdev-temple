...
export default function TempleForm({ mode, templeId, initialValues, onSaved }: Props) {
  ...
  return <form onSubmit={submit} className="space-y-6">
    ...
    <Section title={t.images}><p className="mb-4 text-xs text-gray-500">{t.imagesHelp}</p><div className="grid gap-6 xl:grid-cols-3"><ImageUpload value={values.primaryImage} media={values.primaryMedia} onChange={(value) => setField('primaryImage', value)} onMediaChange={(media) => setField('primaryMedia', media)} folder="sarvdev/temples" label={t.primaryImage} guidance="general" kind="temple-photo" language={hi ? 'hi' : 'en'} /><ImageUpload value={values.imageCard} media={values.cardMedia} onChange={(value) => setField('imageCard', value)} onMediaChange={(media) => setField('cardMedia', media)} folder="sarvdev/temples" label={t.cardImage} guidance="card" kind="temple-photo" language={hi ? 'hi' : 'en'} /><ImageUpload value={values.imageHero} media={values.heroMedia} onChange={(value) => setField('imageHero', value)} onMediaChange={(media) => setField('heroMedia', media)} folder="sarvdev/temples" label={t.heroImage} guidance="hero" kind="temple-photo" language={hi ? 'hi' : 'en'} /></div></Section>
